...
func Run(ctx context.Context, cfg config.OperatorConfig) error {
	...
	staleController := stalecontroller.NewStaleController(cfg, newBugzillaClient(&cfg), slackDebugClient, recorder)

	staleResetController := resetcontroller.NewResetStaleController(cfg, newBugzillaClient(&cfg), slackDebugClient, slackDebugClient, recorder)

	// close stale controller automatically close bugs that were not updated after marked LifecycleClose for 7 days
	closeStaleController := closecontroller.NewCloseStaleController(cfg, newBugzillaClient(&cfg), slackDebugClient, slackDebugClient, recorder)
	...
	blockerReporter := blockers.NewBlockersReporter(cfg, blockerReportSchedule, newBugzillaClient(&cfg), slackDebugClient, slackDebugClient, recorder)
	...
	closedReporter := closed.NewClosedReporter(cfg, closedReportSchedule, newBugzillaClient(&cfg), slackDebugClient, recorder)
